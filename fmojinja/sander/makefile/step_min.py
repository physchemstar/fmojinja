...
\tsander -O -i ${@:%.restrt=%.mdin} -o ${@:%.restrt=%.mdout} -p {{ p }} -c ${<:%.restrt=%.restrt} -ref {{ c }} -r $@ -inf ${@:%.restrt=%.mdinfo}  
{%- else %}
min{{ loop.index }}.restrt:
\tsander -O -i ${@:%.restrt=%.mdin} -o ${@:%.restrt=%.mdout} -p {{ p }} -c {{ c }} -ref {{ c }} -r $@ -inf ${@:%.restrt=%.mdinfo}
{%- endif %}
{%- endfor %}
